...
namespace orch.wf
{
    public abstract class WfHandlerBase : IWfHandler
    {
        ...
        public virtual RuleCheckResult IsActionTypeAvialable(Guid? thisTaskId, OTask task, WfStateData wfState, Guid actionTypeId)
        {
            ...
            {
                throw new InvalidConfigurationError($"Error evaluating CheckAction for {this.GetType()}, actionTypeId:{OTransactionService.GetTypeInfoById(actionTypeId).Type}", ex);
            }
        }
        public virtual RuleCheckResult IsActionTypeAvialableForUser(Guid? thisTaskId, OTask task, WfStateData wfState, UserInfo user, Guid actionTypeId)
        {
            ...
            {
                throw new InvalidConfigurationError($"Error evaluating CheckUser for {this.GetType()}, actionTypeId:{OTransactionService.GetTypeInfoById(actionTypeId).Type}", ex);
            }
        }
        public virtual RuleCheckResult IsActionAvialableForUser(Guid? thisTaskId, OTask task, WfStateData wfState, UserInfo user, Guid actionTypeId, object action)
        {
            ...
            {
                throw new InvalidConfigurationError($"Error evaluating CheckUserAction for {this.GetType()}, actionTypeId:{OTransactionService.GetTypeInfoById(actionTypeId).Type}", ex);
            }
        }
